...
func ParseDir(path string, filterFiles func(fs.FileInfo) bool) ([]*pkgTypes.GoFile, error) {
	...
	goFiles := make([]*pkgTypes.GoFile, 0)
...
func parseFile(path string, file *ast.File, fset *token.FileSet, files []*ast.File) (*pkgTypes.GoFile, error) {
	...
	goFile := &pkgTypes.GoFile{
		Path:    path,
		Package: file.Name.Name,
		Structs: []*pkgTypes.GoStruct{},
	}

	if importPath, err := parseImportPath(filepath.Dir(path)); err != nil {
		return nil, fmt.Errorf("failed to parse import path: %v", err)
	} else {
		goFile.ImportPath = importPath
	}
...
func buildGoVariable(_ *pkgTypes.GoFile, info *types.Info, spec *ast.ValueSpec) *pkgTypes.GoType {
	var t *pkgTypes.GoType
...
func buildGoConstant(_ *pkgTypes.GoFile, info *types.Info, spec *ast.ValueSpec) *pkgTypes.GoType {
	...
	if spec.Type != nil { // untyped const
		...
		t = &pkgTypes.GoType{Type: goTypeStringFromInterface(spec.Names[0].Obj.Data)}
	}
...
func buildGoImport(spec *ast.ImportSpec, file *pkgTypes.GoFile) *pkgTypes.GoImport {
	name := ""
	if spec.Name != nil {
		name = spec.Name.Name
	}
...
func buildGoInterface(file *pkgTypes.GoFile, info *types.Info, typeSpec *ast.TypeSpec, interfaceType *ast.InterfaceType, cg *ast.CommentGroup) *pkgTypes.GoInterface {
	goInterface := &pkgTypes.GoInterface{
		File:     file,
...
func buildMethodList(info *types.Info, fieldList []*ast.Field) []*pkgTypes.GoMethod {
	methods := []*pkgTypes.GoMethod{}
...
func buildStructMethod(info *types.Info, funcDecl *ast.FuncDecl, cg *ast.CommentGroup) *pkgTypes.GoStructMethod {
	return &pkgTypes.GoStructMethod{
...
func buildTypeList(info *types.Info, fieldList *ast.FieldList) []*pkgTypes.GoType {
	types := []*pkgTypes.GoType{}
...
func copyType(goType *pkgTypes.GoType) *pkgTypes.GoType {
	return &pkgTypes.GoType{
		Type:       goType.Type,
		Inner:      goType.Inner,
		Name:       goType.Name,
...
func buildType(info *types.Info, expr ast.Expr) *pkgTypes.GoType {
	innerTypes := []*pkgTypes.GoType{}
...
func buildGoStruct(file *pkgTypes.GoFile, info *types.Info, typeSpec *ast.TypeSpec, structType *ast.StructType, cg *ast.CommentGroup) *pkgTypes.GoStruct {
	goStruct := &pkgTypes.GoStruct{
		File:     file,
		Name:     typeSpec.Name.Name,
		Fields:   []*pkgTypes.GoField{},
		Comments: extractComment(cg),
	}
...
func parseImportPath(dirPath string) (string, error) {
	srcDir, err := filepath.Abs(dirPath)
	if err != nil {
		return "", fmt.Errorf("failed getting source directory: %v", err)
	}
	...
	for {
		...
		if os.IsNotExist(err) {
			if currentDir == filepath.Dir(currentDir) {
				// at the root, but no go.mod found error out
				break
			}
	...
	return "", errors.New("failed to find module path")
